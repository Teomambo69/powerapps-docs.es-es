...
2. En **Create an app** (Crear aplicación), seleccione **Diseño de teléfono** en el icono **Aplicación vacía**.
   ...
   * *Lista de propiedades*:  Cada pantalla y el control tienen un [conjunto de propiedades](reference-properties.md).  Use esta lista para seleccionar una propiedad específica.  
   * *Fórmula*:  La fórmula que se calculará para esta propiedad, compuesto por [valores, operadores y funciones](formula-reference.md).
...
En Excel, el usuario escribe **= A1 + A2** en una celda para mostrar la suma de todos los valores de las celdas **A1** y **A2** contienen. Si uno o ambos de esos valores cambian, la celda que contiene la fórmula muestra automáticamente el resultado actualizado.

![Animación de volver a calcular la suma de dos números de Excel](./media/working-with-formulas/excel-recalc.gif)

En PowerApps, puede lograr un resultado similar al agregar controles a una pantalla y establecer sus propiedades. En este ejemplo se muestra un control label denominado **Label1** y dos **[entrada de texto](controls/control-text-input.md)** controles, denominados **TextInput1** y  **TextInput2**.

![Ilustración de volver a calcular la suma de dos números de PowerApps](./media/working-with-formulas/recalc1.png)
...
![Animación de volver a calcular la suma de dos números de PowerApps](./media/working-with-formulas/recalc2.gif)

En Excel, puede usar el formato condicional fórmulas para mostrar, por ejemplo, los valores negativos en rojo. En PowerApps, puede usar fórmulas para determinar no solo el valor principal de un control, sino propiedades como el formato. En el ejemplo siguiente, una fórmula para el **[Color](controls/properties-color-border.md)** propiedad de la etiqueta muestra automáticamente los valores negativos en rojo. El aspecto de la función **[If](functions/function-if.md)** debería resultarle familiar de Excel:
