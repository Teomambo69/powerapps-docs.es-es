...
Mediante el uso de la **Color** enumeración, se pueden acceder fácilmente a los colores que se definen por hojas de estilos de en cascada de HTML (CSS). Por ejemplo, **Color.Red** devuelve rojo puro. Puede encontrar una lista de estos colores al final de este tema.

El **ColorValue** función devuelve un color basado en una cadena de color de CSS. La cadena puede tener cualquiera de estas formas:

- **Nombre del color CSS:** **"RoxyBrown"** y **"OliveDrab"** son ejemplos. Estos nombres no incluyen espacios. Aparece la lista de colores compatibles más adelante en este tema.
- **valor hexadecimal de 6 dígitos:** Por ejemplo **"#ffd700"** es el mismo que **"Gold"** . La cadena tiene el formato "#*rrggbb*" donde *rr* es la parte roja de dos dígitos hexadecimales, *gg* es verde, y *bb* es azul.
- **valor hexadecimal de 8 dígitos:** Por ejemplo, **"#ff7f5080"** es el mismo que **"Coral"** con un canal alfa del 50%. La cadena tiene el formato "#*rrggbbaa*" donde *rr*, *gg*, y *bb* son idénticas a la forma de 6 dígitos. El canal alfa se representa mediante *aa*: **00** representa totalmente transparente, y **ff** representa totalmente opaco.

El **RGBA** función devuelve un color basado en componentes rojo, verde y azules. La función también incluye un canal alfa para mezclar colores de los controles que se superponen delante de uno a otro. Un canal alfa varía de 0 o 0% (que es completamente transparente e invisible) a 1 o 100% (que es completamente opaco y bloquea totalmente las capas detrás de un control).

La función **ColorFade** devuelve una versión más clara o más oscura de un color. La cantidad de atenuación varía de -1 (que oscurece completamente un color a negro) en 0 (lo que no afecta al color) a 1 (que ilumina totalmente un color en blanco).

## <a name="alpha-channel"></a>Canal alfa

En una aplicación de lienzo, puede controles delante de uno a otro de capas y especificar la transparencia de un control para los controles que están detrás de él. Como resultado, los colores se mezclarán a través de las capas. Por ejemplo, este diagrama muestra cómo combinar estos tres colores primarios con un valor alfa del 50%:

> [!div class="mx-imgBorder"]
> ![Tres colores primarios con un valor alfa del 50%](media/function-colors/alpha-primary.png)

También puede combinar las imágenes en formatos de archivo que admiten canales alfa. Por ejemplo, no se pueden fusionar .jpeg (archivos), pero puede combinar archivos .png. El gráfico siguiente muestra la misma red, los colores verdes y azules del ejemplo anterior, pero el color rojo aparece como un subrayado ondulado de color (en lugar de un círculo) en un archivo .png con un canal alfa del 50%:

> [!div class="mx-imgBorder"]
> ![Subrayado ondulado de color rojo con un valor alfa del 50% delante de los círculos azules y verdes](media/function-colors/alpha-image.png)

Si especifica un **Color** valor de enumeración, o bien crear una **ColorValue** fórmula con un nombre de color o un valor hexadecimal de 6 dígitos, el valor alfa es 100%, que es completamente opaco.

## <a name="syntax"></a>Sintaxis

**Color**.*ColorName*

- *ColorName*: requerido.  El nombre de un color de hoja de estilo CSS. Aparece la lista de posibles valores de enumeración al final de este tema.

**ColorValue**( *CSSColor* )

- *CSSColor*: requerido.  La definición de un color de hoja de estilo CSS. Puede especificar un nombre, como **OliveDrab**, o un valor hexadecimal, como **#6b8e23** o **7fffd420 #** . Los valores hexadecimales pueden adoptar la forma de cualquier #*rrggbb* o #*rrggbbaa*.

**RGBA**( *Red*, *Green*, *Blue*, *Alpha* )

- *Red*, *Green*, *Blue*: requerido.  Valores de componente de color, que van de 0 (sin saturación) a 255 (saturación total).
- *Alpha*: requerido.  Componente alfa, comprendido entre 0 (completamente transparente) y 1 (completamente opaco). También puede usar un porcentaje, de 0 % a 100 %.

**ColorFade**( *Color*, *FadeAmount* )

- *Color*: requerido.  Un valor de color como **Color.Red** o el resultado de **ColorValue** o **RGBA**.
- *FadeAmount*: requerido.  Número entre -1 y 1. -1 oscurece completamente un color a negro, 0 no afecta al color y 1 ilumina totalmente un color en blanco. También puede usar un porcentaje entre -100% a 100%.
...
| **Color.DarkSeaGreen** |**ColorValue( "#8fbc8f"&nbsp;)**<br>**ColorValue ("Encaje antiguo"&nbsp;)** |**RGBA(&nbsp;143,&nbsp;188,&nbsp;143,&nbsp;1&nbsp;)** |![darkseagreen](./media/function-colors/color-darkseagreen.png) |
...
| **Color.OrangeRed** |**ColorValue( "#ff4500"&nbsp;)**<br>**ColorValue ("Rojo ANARANJADO"&nbsp;)** |**RGBA(&nbsp;255,&nbsp;69,&nbsp;0,&nbsp;1&nbsp;)** |![orangered](./media/function-colors/color-orangered.png) |
...
| **Color.Transparent** |**ColorValue( "#00000000"&nbsp;)**<br>**ColorValue( "Transparent"&nbsp;)** |**RGBA(&nbsp;0,&nbsp;0,&nbsp;0,&nbsp;0&nbsp;)** |![Transparente](./media/function-colors/color-transparent.png) |
...
| **Color.YellowGreen** |**ColorValue( "#9acd32"&nbsp;)**<br>**ColorValue ("Verde AMARILLENTO"&nbsp;)** |**RGBA(&nbsp;154,&nbsp;205,&nbsp;50,&nbsp;1&nbsp;)** |![yellowgreen](./media/function-colors/color-yellowgreen.png) |
